    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function __construct($redis, $config=array())
    {
        ...
        $this->config = array_merge(array(
            ...
            'global_oauth_login_user_key' => 'global_oauth_login_user:'
        ), $config);
    }
    ...
    //2018-02-05 update
    public function setUser($user_id,$username, $password, $first_name = null, $last_name = null)
    {
        return $this->setValue(
            $this->config['user_key'] . $username,
            compact('user_id','username', 'password', 'first_name', 'last_name')
        );
    }
    ...
    public function setClientDetails($client_id, $client_secret = null, $redirect_uri = null, $grant_types = null, $scope = null, $issso = null,$logout_url=null,$isrefresh_token,$viewType)
    {
       /* return $this->setValue(
        ...
        return $this->setValue(
            $this->config['client_key'] . $client_id,
            compact('client_id', 'client_secret', 'redirect_uri', 'grant_types', 'scope', 'issso','logout_url','isrefresh_token','viewType')
        );
    }
     ...
     * @param $type_name
     */
    public function setOauthLoginUserFromRedis($openid,$nickname,$avatar,$sex,$expires)
    {

        return $this->setValue(
            $this->config['global_oauth_login_user_key'] . $openid,
            compact('openid', 'nickname', 'avatar', 'sex'),
            $expires
        );
    }

    /**
     * 根据openid 获取redis用户数据
     * @param $openid
     * @return bool|mixed
     */
    public function getOauthLoginUserFromRedis_openid($openid)
    {
        return $this->getValue($this->config['global_oauth_login_user_key'].$openid);
    }

    /**
     * 根据userid 获取redis存储的基本用户信息
     * @param $userid
     * @return bool|mixed
     */
    public function getPstuserByUseidFromUserinfo($userid)
    {
